...
マイルストーンごとのグループは、デフォルトではid順に並びます。今どれを進めているかに合わせて順番を変えたいときは、一覧の見出しかサイドバーのマイルストーンにマウスを乗せると、ドラッグ用のハンドルと上下ボタンが出てきます。どちらから並び替えても、一覧とサイドバーの両方に反映されます。

並び順は`.rrmap/milestones/order.json`にidの配列として保存されるので、直接書き換えてもかまいません（[マイルストーンファイルのフォーマット](docs/milestone-format.md)参照）。

### タスクの並び替え

マイルストーンの中のタスクも同じように並び替えられます。タスクの行にマウスを乗せると、IDの左にドラッグ用のハンドルと上下ボタンが出てきます。子タスクは親タスクについていくので、並び替えの対象になるのは同じ階層のタスク同士です。

並び順は`.rrmap/tasks/order.json`にidの配列として保存されます（[タスクファイルのフォーマット](docs/task-format.md)参照）。
